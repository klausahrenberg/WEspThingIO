...
class W2812Led : public WOutput {
 public:
  W2812Led(WNetwork* network, int ledPin, byte numberOfLeds) : WOutput(ledPin) {    
    ...
    _brightness = new WLevelIntProperty("brightness", "Brightness", 10, 255);
  ...
  WLevelIntProperty* brightness() { return _brightness; }
          ...
          case 2: {
            ...
            for (uint16_t rainbowI = 0; rainbowI < _strip->numPixels();
              ...
              _strip->setPixelColor(
                  rainbowI, wheelColor(((rainbowI * 256 / _strip->numPixels()) +
                                        _programStatusCounter) &
                                       255));
            }
            _strip->show();
